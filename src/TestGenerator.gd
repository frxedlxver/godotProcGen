...
@export var converter : ImageToTilemapConverter
@export var tree_placer : TreePlacer
...
func _ready():
	...
	#_updateTexture()
	converter.image_to_tilemap(image)
	zoom_to_fit_tilemap(image.get_size(), 32)
...
func generate_map(map_size : Vector2i):
		...
		var tree_locations = flora_gen.generate_flora(landscape)
		tree_placer.place_trees_at_points(tree_locations)
		image = landscape
